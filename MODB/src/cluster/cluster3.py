...
def splits(args, gb_dict, select):
    """
    按纯度与最小球规模参数迭代分裂粒球。
    - select=False：使用 purity_train、min_ball_train。当 纯度 < purity_train 且 样本数 > min_ball_train 时分裂。
    - select=True：使用 purity_get_ball、min_ball_get_ball。规则同上。
    分裂过程完全由 args 中上述四个参数控制。
    """
    ...
    while True:
        ...
        if p is not None and p < a_purity and number_of_samples_in_ball > a_min_ball:
...
def main(args, data, select):
    """
    粒球生成入口：初始化一个球后调用 splits 分裂，再按选球参数过滤输出。
    - 分裂阶段：由 splits(args, gb_dict, select) 使用 purity_* / min_ball_* 控制。
    - 选球阶段：select=False 用 purity_select_ball_train、min_ball_select_ball_train；
                select=True 用 purity_select_ball、min_ball_select_ball。只保留纯度与样本数满足条件的球。
    """
    ...
    result = []
    if select == False:
        len_ball = args.min_ball_select_ball_train
        b_purity = args.purity_select_ball_train
    else:
    ...
    return numbers, result, centers, max_radius

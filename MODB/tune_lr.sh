#!/usr/bin/env bash
# 自动调参脚本：主要调 lr / lr2 / lr3，参数会写入 outputs/results.csv
# 用法: bash tune_lr.sh
# 快速少组合: QUICK=1 bash tune_lr.sh
set -e
cd "$(dirname "$0")"
...
# 学习率候选（可改）
# lr: 表示学习  lr2: 最近子质心/多粒度球  lr3: 边界损失
if [ "${QUICK}" = "1" ]; then
  LRS=(2e-5)
  LR2S=(1e-4)
  LR3S=(1e-4)
...
echo "Done. Results with lr/lr2/lr3 saved to outputs/results.csv"
